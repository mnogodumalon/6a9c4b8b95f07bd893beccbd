...
export type MitgliederDialogDefaults = Omit<Mitglieder['fields'], 'status' | 'rollen'> & {
    status?: LookupValue | string;
    rollen?: (LookupValue | string)[];
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  ...
  rollen: LOOKUP_OPTIONS['mitglieder']?.['rollen'] ?? [],
};
...
export function MitgliederDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: MitgliederDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "vorname": string | null, // Vorname\n  "nachname": string | null, // Nachname\n  "mitgliedsnummer": string | null, // Mitgliedsnummer\n  "geburtsdatum": string | null, // YYYY-MM-DD\n  "email": string | null, // E-Mail-Adresse\n  "telefon": string | null, // Telefonnummer\n  "eintrittsdatum": string | null, // YYYY-MM-DD\n  "status": LookupValue | null, // Status (select one key: "aktiv" | "inaktiv" | "pausiert") mapping: aktiv=Aktiv, inaktiv=Inaktiv, pausiert=Pausiert\n  "rollen": LookupValue[] | null, // Rollen im Verein (select one or more keys: "ruderer" | "steuermann" | "bootswart" | "trainer" | "administrator" | "vorstand") mapping: ruderer=Ruderer, steuermann=Steuermann, bootswart=Bootswart, trainer=Trainer, administrator=Administrator, vorstand=Vorstand\n  "notizen_mitglied": string | null, // Interne Notizen\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'mitgliedsnummer': (
      <div key="mitgliedsnummer" className="space-y-1.5">
        <Label htmlFor="mitgliedsnummer">{fieldLabel('mitglieder', 'mitgliedsnummer')}</Label>
        <Input
          id="mitgliedsnummer"
    ...
    'geburtsdatum': (
      <div key="geburtsdatum" className="space-y-1.5">
        <Label htmlFor="geburtsdatum">{fieldLabel('mitglieder', 'geburtsdatum')}</Label>
        <DatePicker
          id="geburtsdatum"
    ...
    'eintrittsdatum': (
      <div key="eintrittsdatum" className="space-y-1.5">
        ...
        <DatePicker
          id="eintrittsdatum"
          placeholder="Wann tritt das Mitglied ein?"
          mode="date"
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('mitglieder', 'status', 'pausiert') ?? 'Pausiert'}
          </button>
        </div>
    ...
    'rollen': (
      <div key="rollen" className="space-y-1.5">
        <Label htmlFor="rollen">{fieldLabel('mitglieder', 'rollen')}</Label>
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <Checkbox
              id="rollen_ruderer"
              checked={lookupKeys(fields.rollen).includes('ruderer')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.rollen);
                  const next = checked ? [...current, 'ruderer'] : current.filter(k => k !== 'ruderer');
                  return { ...f, rollen: next.length ? next as any : undefined };
                });
              }}
            />
            <Label htmlFor="rollen_ruderer" className="font-normal">{lookupLabel('mitglieder', 'rollen', 'ruderer') ?? 'Ruderer'}</Label>
          </div>
          <div className="flex items-center gap-2">
            <Checkbox
              id="rollen_steuermann"
              checked={lookupKeys(fields.rollen).includes('steuermann')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.rollen);
                  const next = checked ? [...current, 'steuermann'] : current.filter(k => k !== 'steuermann');
                  return { ...f, rollen: next.length ? next as any : undefined };
                });
              }}
            />
            <Label htmlFor="rollen_steuermann" className="font-normal">{lookupLabel('mitglieder', 'rollen', 'steuermann') ?? 'Steuermann'}</Label>
          </div>
          <div className="flex items-center gap-2">
            <Checkbox
              id="rollen_bootswart"
              checked={lookupKeys(fields.rollen).includes('bootswart')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.rollen);
                  const next = checked ? [...current, 'bootswart'] : current.filter(k => k !== 'bootswart');
                  return { ...f, rollen: next.length ? next as any : undefined };
                });
              }}
            />
            <Label htmlFor="rollen_bootswart" className="font-normal">{lookupLabel('mitglieder', 'rollen', 'bootswart') ?? 'Bootswart'}</Label>
          </div>
          <div className="flex items-center gap-2">
            <Checkbox
              id="rollen_trainer"
              checked={lookupKeys(fields.rollen).includes('trainer')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.rollen);
                  const next = checked ? [...current, 'trainer'] : current.filter(k => k !== 'trainer');
                  return { ...f, rollen: next.length ? next as any : undefined };
                });
              }}
            />
            <Label htmlFor="rollen_trainer" className="font-normal">{lookupLabel('mitglieder', 'rollen', 'trainer') ?? 'Trainer'}</Label>
          </div>
          <div className="flex items-center gap-2">
            <Checkbox
              id="rollen_administrator"
              checked={lookupKeys(fields.rollen).includes('administrator')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.rollen);
                  const next = checked ? [...current, 'administrator'] : current.filter(k => k !== 'administrator');
                  return { ...f, rollen: next.length ? next as any : undefined };
                });
              }}
            />
            <Label htmlFor="rollen_administrator" className="font-normal">{lookupLabel('mitglieder', 'rollen', 'administrator') ?? 'Administrator'}</Label>
          </div>
          <div className="flex items-center gap-2">
            <Checkbox
              id="rollen_vorstand"
              checked={lookupKeys(fields.rollen).includes('vorstand')}
              onCheckedChange={(checked) => {
                setFields(f => {
                  const current = lookupKeys(f.rollen);
                  const next = checked ? [...current, 'vorstand'] : current.filter(k => k !== 'vorstand');
                  return { ...f, rollen: next.length ? next as any : undefined };
                });
              }}
            />
            <Label htmlFor="rollen_vorstand" className="font-normal">{lookupLabel('mitglieder', 'rollen', 'vorstand') ?? 'Vorstand'}</Label>
          </div>
        </div>
      </div>
    ),
    'notizen_mitglied': (
      <div key="notizen_mitglied" className="space-y-1.5">
        <Label htmlFor="notizen_mitglied">{fieldLabel('mitglieder', 'notizen_mitglied')}</Label>
        <Textarea
          id="notizen_mitglied"
          placeholder="Besonderheiten, Fähigkeiten, Anmerkungen..."
          value={fields.notizen_mitglied ?? ''}
          onChange={e => setFields(f => ({ ...f, notizen_mitglied: e.target.value }))}
          rows={3}
        />
      </div>
    ),
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "mitgliedsnummer": "Mitgliedsnummer", "geburtsdatum": "Geburtsdatum", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "eintrittsdatum": "Eintrittsdatum", "status": "Status", "rollen": "Rollen im Verein", "notizen_mitglied": "Interne Notizen"};
