          ...
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('mitgliedsnummer')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('mitglieder', 'mitgliedsnummer')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('geburtsdatum')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('mitglieder', 'geburtsdatum')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('rollen')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('mitglieder', 'rollen')}
                  {sortKey === 'rollen' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('notizen_mitglied')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('mitglieder', 'notizen_mitglied')}
                  {sortKey === 'notizen_mitglied' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/mitglieder/${record.record_id}`); }}>
                ...
                <TableCell>{Array.isArray(record.fields.rollen) ? record.fields.rollen.map((v: any) => lookupLabel('mitglieder', 'rollen', v?.key) ?? v?.label ?? v).join(', ') : '—'}</TableCell>
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.notizen_mitglied ?? '—'}</span></TableCell>
                <TableCell>
